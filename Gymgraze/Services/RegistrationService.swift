...
class RegistrationService {
    ...
    func confirmEmail(confirmationToken: String, completion: @escaping (Result<Date, APIError>) -> Void) {
        ...
        // construct the URL
        guard let url = URL(string: "http://rattler-amusing-explicitly.ngrok-free.app/confirm_email") else {
            // if it's not valid, throw a invalid URL error
            completion(.failure(APIError.invalidURL) as Result<Date, APIError>)
            return
        }
    ...
    func resendEmailConfirmation(completion: @escaping (Result<Bool, APIError>) -> Void) {
        ...
        // construct the URL
        guard let url = URL(string: "http://rattler-amusing-explicitly.ngrok-free.app/resend_confirmation_email") else {
            // if it's not valid, throw a invalid URL error
